                      ...
                      SDL_Renderer * renderer, const char *restrict const locale)
{
  ...
  if (socketpair(AF_UNIX, SOCK_STREAM, 0, sv))
    ...
    int backend_fd = sv[1];
    int frontend_fd = sv[0];

    dispatch_async(q, ^{
        // ==== Begin "child process" logic ====
        font_socket_fd = backend_fd;
        close(frontend_fd);
        ...
        int size = 0;
        for (int i = num_font_families - 1; i >= 0; i--) {
            char *s;
            ...
            if (s) size += strlen(s);
            for (int j = 0; j < 4; j++) {
                s = user_font_families[i]->filename[j];
                if (s) size += strlen(s);
            }
            size += 6; // null terminators
        }
        size += 2; // font count
        ...
        *walk++ = num_font_families >> 8u;
        ...
        for (int i = num_font_families - 1; i >= 0; i--) {
            char *s;
            s = user_font_families[i]->directory;
            if (s) { memcpy(walk, s, strlen(s)); walk += strlen(s); }
            *walk++ = '\0';
