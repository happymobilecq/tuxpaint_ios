...
#ifdef __IPHONEOS__
    if (tool < NUM_TOOLS && tool != TOOL_QUIT && tool != TOOL_PRINT)
#else
    if (tool < NUM_TOOLS)
#endif
    {
...
#define MAGIC_NAME(PN) PN,
enum   {
#include "magic_name.h"
    plugin_num
} PLUGINS;
#undef MAGIC_NAME
...
#define DECLARE_MAGIC_FUNC(PN)\
    ...
    int magic_command(PN,get_group)(magic_api *, int);\
    ...
    Uint8 magic_command(PN,accepted_sizes) (magic_api *, int, int);\
    Uint8 magic_command(PN,default_size) (magic_api *, int, int);\
    int magic_command(PN,modes)(magic_api *, int);\
    void magic_command(PN,set_color)(magic_api *, int, SDL_Surface *, SDL_Surface *, Uint8, Uint8, Uint8, SDL_Rect *);\
    void magic_command(PN,set_size)(magic_api *, int, int, SDL_Surface *, SDL_Surface *, Uint8, SDL_Rect *);\
    int magic_command(PN,init)(magic_api *, Uint32);\
...
#define MAGIC_FUNC_POINTER(PN) \
    ...
    magic_funcs[PN].get_group                       = magic_command(PN, get_group);\
    ...
    magic_funcs[PN].accepted_sizes = magic_command(PN, accepted_sizes);\
    magic_funcs[PN].default_size = magic_command(PN, default_size);\
    magic_funcs[PN].modes= magic_command(PN, modes);\
    magic_funcs[PN].set_color = magic_command(PN, set_color);\
    magic_funcs[PN].set_size = magic_command(PN, set_size);\
...
#define MAGIC_NAME(PN) MAGIC_FUNC_POINTER(PN)
...
static void load_magic_plugins(void)
{
    ...
    for (num_plugin_files = 0; num_plugin_files < plugin_num; num_plugin_files++) {
        ...
        if (magic_funcs[num_plugin_files].api_version() != TP_MAGIC_API_VERSION)
        {
          fprintf(stderr,
                  "Warning: plugin %d uses Tux Paint 'Magic' tool API version %x,\nbut Tux Paint needs version %x.\n",
                  num_plugin_files, magic_funcs[num_plugin_files].api_version(), TP_MAGIC_API_VERSION);
          err = 1;
          continue;
        }
          ...
          else
          {
            ...
            for (i = 0; i < n; i++)
            {
              ...
              if (group < MAX_MAGIC_GROUPS)
              {
                ...
                for (j = 0; j < MAX_MODES; j++)
                {
                  ...
                  if (mode_bit != 0)
                  {
                    ...
                    if (magics[group][idx].sizes[j] > 1)
                    {
                      ...
                      {
                        fprintf(stderr,
                                "Warning: plugin %d tool # %d for %d mode (%x) default size (%d) out of range (1-%d)\n",
                                num_plugin_files, i, j, mode_bit, magics[group][idx].default_size[j],
                                magics[group][idx].sizes[j]);
                ...
                {
                  fprintf(stderr, "Error: plugin %d mode # %d failed to load an icon\n", num_plugin_files, i);
                  fflush(stderr);
                }
              }
              else
              {
                fprintf(stderr,
                        "Error: plugin %d mode # %d reported group %d (higher than %d)\n",
                        num_plugin_files, i, group, MAX_MAGIC_GROUPS - 1);
                fflush(stderr);
              }
            }
...
static void setup(void)
{
  ...
  safe_snprintf(tmp_str, sizeof(tmp_str), "© 2002–2025 Bill Kendrick, et al.");
  tmp_surf = render_text(medium_font, tmp_str, black);
  dest.x = 10;
...
#ifdef __IPHONEOS__
  safe_snprintf(fname, sizeof(fname), "giftmp1234.%s", ext);
#else
  /* Create a unique filename, within that dir */
  t = time(NULL);
  strftime(timestamp, sizeof(timestamp), "%Y%m%d%H%M%S", localtime(&t));
